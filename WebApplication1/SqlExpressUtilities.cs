...
namespace SqlExpressUtilities
{
	...
	public class SqlExpressWrapper
	{
        ...
        public virtual bool SelectWhere(string Column, string Thing)
        {
            string sql = "Select * FROM " + SQLTableName + " WHERE " + Column + " = '" + Thing + "'";
            QuerySQL(sql);
            return reader.HasRows;
        }

        public virtual bool SelectFromStringies(string[] stringies)
        {
            // Malheureusement, order by n'existe pas en SQL
            string sql = "SELECT * FROM " + SQLTableName /*+ " ORDER BY PRICE ASC "*/;
            bool set = false;

            // C'est tu plus slow qu'in 'for'?
            foreach (string stringy in stringies)
            {
                string maj = stringy.ToUpper();
                string min = stringy.ToLower();
                string m1j = stringy;

                if (stringy.Length > 1)
                {
                    m1j = char.ToUpper(m1j[0]) + m1j.Substring(1);
                }

                if (set == false) // if ! set, set veut dire qu'on a déjà mit de quoi. 
                {
                    if (stringy != "" && stringy != " ")
                    {
                        sql += " WHERE SUMMARY LIKE '%" + maj + "%' OR SUMMARY LIKE '%" + min + "%' OR SUMMARY LIKE '%" + m1j + "%'";
                        set = true;
                    }
                }
                else
                {
                    ...
                    if (stringy != "" && stringy != " ")
                    {
                        sql += " AND SUMMARY LIKE '%" + maj + "%' OR SUMMARY LIKE '%" + min + "%' OR SUMMARY LIKE '%" + m1j + "%'";
                        /*set = true;*/
                        // Pas besoin de le setter encore une fois
                    }
                }
            }

            // ça non plus ça marche pas. on est limité par SQL.
            //sql = "SELECT * FROM (" + sql + ") ORDER BY PRICE ASC";
